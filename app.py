...
def calculate_foundation_weight(params, rho_concrete):
    """
    Calculate the total weight of the foundation and individual components.
    
    Parameters:
    - params: list of dimensions [d1, d2, h1, h2, h3, h4, h5, b1, b2]
    - rho_concrete: density of concrete (kN/m³)
    
    Returns:
    - total_weight: total weight of the foundation (kN)
    - C1, C2, C3, C4: volumes of different foundation components (m³)
    """
    ...
    total_weight = (C1 + C2 + C3 + C4) * rho_concrete
...
def calculate_ballast_and_buoyancy(params, C2, C4, rho_ballast_wet, rho_water):
    ...
    Parameters:
    - params: list of dimensions [d1, d2, h1, h2, h3, h4, h5]
    - C2, C4: volumes of specific components (m³)
    - rho_ballast_wet: wet density of ballast (kN/m³)
    - rho_water: density of water (kN/m³)
    ...
    return B_wet, buoyancy

def net_vertical_load(params, F_z, rho_concrete, rho_ballast_wet, rho_water):
    """
    Calculate the net vertical load on the foundation.
    
    Parameters:
    - params: list of dimensions [d1, d2, h1, h2, h3, h4, h5, b1, b2]
    - F_z: vertical load (kN)
    - rho_concrete: density of concrete (kN/m³)
    - rho_ballast_wet: wet density of ballast (kN/m³)
    - rho_water: density of water (kN/m³)
    
    Returns:
    - net_load: net vertical load (kN)
    - total_weight, B_wet, buoyancy: intermediate calculation results (kN)
    """
    total_weight, C1, C2, C3, C4 = calculate_foundation_weight(params, rho_concrete)
    B_wet, buoyancy = calculate_ballast_and_buoyancy(params, C2, C4, rho_ballast_wet, rho_water)
    net_load = buoyancy + B_wet + total_weight + F_z
    return net_load, total_weight, B_wet, buoyancy

def calculate_pressures(params, F_z, F_res, M_res, rho_concrete, rho_ballast_wet, rho_water):
    """
    ...
    Returns:
    - p_min, p_max: minimum and maximum pressures (kN/m²)
    - B_wet, buoyancy, vertical_load, total_weight: intermediate calculation results (kN)
    """
    d1 = params[0]
    vertical_load, total_weight, B_wet, buoyancy = net_vertical_load(params, F_z, rho_concrete, rho_ballast_wet, rho_water)
    resultant_moment = M_res + F_res * (params[2] + params[3] + params[4])
    ...
    return p_min, p_max, B_wet, buoyancy, vertical_load, total_weight

def plot_foundation_comparison(original_params, optimized_params):
    """
    Plot a comparison of the original and optimized foundation geometries.
    
    Parameters:
    - original_params: list of original dimensions
    - optimized_params: list of optimized dimensions
    ...
    def plot_foundation(params, edgecolor, fillcolor):
        d1, d2, h1, h2, h3, h4, h5, b1, b2 = params
    ...
    plot_foundation(original_params, 'black', 'rgba(128, 128, 128, 0.5)')
    plot_foundation(optimized_params, 'green', 'rgba(144, 238, 144, 0.5)')

    fig.update_layout(
        title="Foundation Comparison",
        xaxis=dict(title="Width (m)", range=[-15, 15]),
        yaxis=dict(title="Height (m)", scaleanchor="x", scaleratio=1, range=[-1, 4]),
...
def run_calculations(F_z, F_res, M_res, rho_concrete, rho_ballast_wet, rho_water, params):
    """
    Run foundation calculations and return the results.
    ...
    - rho_ballast_wet: wet density of ballast (kN/m³)
    - rho_water: density of water (kN/m³)
    - params: list of dimensions
    
    Returns:
    - result: dictionary of calculated values
    - concrete_volume: total concrete volume (m³)
    """
    total_weight, C1, C2, C3, C4 = calculate_foundation_weight(params, rho_concrete)
    p_min, p_max, B_wet, buoyancy, net_load = calculate_pressures(params, F_z, F_res, M_res, rho_concrete, rho_ballast_wet, rho_water)[:5]

    result = {
        "Parameter": ["d1", "d2", "h1", "h2", "h3", "h4", "h5", "b1", "b2", "C1", "C2", "C3", "C4",
                      "Total weight", "p_min", "p_max", "B_wet", "W", "F_z", "net_load"],
        "Value": [f"{params[0]:.3f} m", f"{params[1]:.3f} m", f"{params[2]:.3f} m", f"{params[3]:.3f} m", f"{params[4]:.3f} m",
                  f"{params[5]:.3f} m", f"{params[6]:.3f} m", f"{params[7]:.3f} m", f"{params[8]:.3f} m",
                  f"{C1:.3f} m³", f"{C2:.3f} m³", f"{C3:.3f} m³", f"{C4:.3f} m³",
                  f"{total_weight:.3f} kN", f"{p_min:.3f} kN/m²", f"{p_max:.3f} kN/m²", f"{B_wet:.3f} kN", f"{buoyancy:.3f} kN",
...
def optimize_foundation(F_z, F_res, M_res, rho_concrete, rho_ballast_wet, rho_water, initial_params, h_anchor):
    """
    Optimize the foundation dimensions to minimize concrete volume.
    ...
    - rho_ballast_wet: wet density of ballast (kN/m³)
    - rho_water: density of water (kN/m³)
    - initial_params: list of initial dimensions
    - h_anchor: anchor height (m)
    
    Returns:
    - result_output: dictionary of optimized values
    - optimized_concrete_volume: optimized concrete volume (m³)
    - fig: plotly figure object
    """
    ...
    def objective(x):
        ...
        _, _, _, _, _, total_weight = calculate_pressures(params, F_z, F_res, M_res, rho_concrete, rho_ballast_wet, rho_water)
        return total_weight

    def constraint_pmin(x):
        params = [x[0], initial_params[1], x[1], x[2], x[3], initial_params[5], initial_params[6], initial_params[7], initial_params[8]]
        p_min, _, _, _, _, _ = calculate_pressures(params, F_z, F_res, M_res, rho_concrete, rho_ballast_wet, rho_water)
    ...
    constraints = [{'type': 'ineq', 'fun': constraint_pmin},
    ...
    try:
        result = minimize(objective, [initial_params[0], initial_params[2], initial_params[3], initial_params[4]],
                          bounds=[bounds[0], bounds[2], bounds[3], bounds[4]], constraints=constraints, method='trust-constr')
...
def plot_3d_foundation(params):
    """
    Plot a 3D representation of the foundation.
    ...
    add_cylinder(d1 / 2, h1, 0, 'gray')  # slab
    add_conical_frustum(d1 / 2, d2 / 2, h2, h1, 'gray')  # haunch
    add_cylinder(d2 / 2, h3, h1 + h2, 'gray', top=True)  # plinth
    add_conical_frustum(b1 / 2, b2 / 2, 0, -h5, 'gray')  # downstand
...
rho_concrete = st.sidebar.number_input(r'$\rho_{conc}$ (kN/m³)', value=24.500, format="%.3f")
rho_ballast_wet = st.sidebar.number_input(r'$\rho_{ballast\,wet}$ (kN/m³)', value=20.000, format="%.3f")
rho_water = st.sidebar.number_input(r'$\rho_{water}$ (kN/m³)', value=9.810, format="%.3f")
...
if st.button("Run Calculations"):
    result_output, original_concrete_volume = run_calculations(F_z, F_RES, M_RES, rho_concrete, rho_ballast_wet, rho_water, initial_params)
...
if st.button("Optimize Foundation"):
    result_output, optimized_concrete_volume, fig = optimize_foundation(F_z, F_RES, M_RES, rho_concrete, rho_ballast_wet, rho_water, initial_params, h_anchor)
    result_df = pd.DataFrame(result_output)
    st.dataframe(result_df.style.hide(axis="index"), use_container_width=True)
    if fig is not None:
